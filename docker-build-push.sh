...
echo "🐳 Docker Build & Push Script para Products Service"
echo "=================================================="
echo ""
...
CONTAINER_ID=$(docker run -d -p 8082:8081 --name products-service-test "$VERSIONED_TAG")
...
if [ $? -eq 0 ]; then
    ...
    if docker ps | grep -q "products-service-test"; then
        ...
        if docker exec products-service-test curl -f http://localhost:8081/actuator/health 2>/dev/null; then
    ...
    docker stop products-service-test >/dev/null 2>&1 || true
    docker rm products-service-test >/dev/null 2>&1 || true
